...
const { isDir, expandHome, wrapText, computeViewport } = require("./util");
const { saveConfig } = require("./config");
const { buildSearchIndex, filterSearchIndex } = require("./scan");
...
const { askLine, openEditor, pickOption } = require("./prompts");
...
async function addProjectFolder(cwd, cfg) {
  const typed = await askLine(
    "Path to project folder (absolute or ~/…, blank = current directory): ",
  );
...
async function createFlow(data, tabKey, imageLogo, spellBase64) {
  ...
  const name = await askLine(
    "New agent name (lowercase-hyphens, e.g. code-reviewer): ",
    spellSlot(),
  );
  ...
  const seniority = await askLine(
    "Seniority / experience level? (e.g. junior, senior, principal): ",
    spellSlot(),
  );
  const tasks = await askLine(
    "General tasks it will perform (a sentence, or comma-separated list): ",
    spellSlot(),
  );
